'''
train script for a periodic unet model with 4 loss terms in the loss function
...
from models.model import CombinedDiceMSELoss, BCE_MSE_loss, Dice_MSE_sseCELL_sseVEGF_loss, BCE_MSE_sseCELL_sseVEGF_loss
...
LOSS_WEIGHT_THREE = float(os.getenv("LOSS_WEIGHT_THREE", "1.0"))
LOSS_WEIGHT_FOUR = float(os.getenv("LOSS_WEIGHT_FOUR", "1.0"))
...
class LitModelPeriodic(pl.LightningModule):
    def __init__(self):
        ...
        self.criterion = Dice_MSE_sseCELL_sseVEGF_loss(dice_weight=LOSS_WEIGHT_ONE, mse_weight=LOSS_WEIGHT_TWO, sseCELL_weight=LOSS_WEIGHT_THREE, sseVEGF_weight=LOSS_WEIGHT_FOUR)
        # self.criterion = BCE_MSE_sseCELL_sseVEGF_loss(bce_weight=LOSS_WEIGHT_ONE, mse_weight=LOSS_WEIGHT_TWO, sseCELL_weight=LOSS_WEIGHT_THREE, sseVEGF_weight=LOSS_WEIGHT_FOUR)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, cell_loss, vegf_loss, cell_constraint, vegf_constraint = self.criterion(y_hat, targets)
        self.log('train_loss', loss, on_epoch=True, sync_dist=True) #sync_dist=True is used for multi-GPU training
        self.log('train_cell_loss', cell_loss, on_epoch=True, sync_dist=True)
        self.log('train_MSE_loss', vegf_loss, on_epoch=True, sync_dist=True)
        self.log('train_cell_constraint', cell_constraint, on_epoch=True, sync_dist=True)
        self.log('train_vegf_constraint', vegf_constraint, on_epoch=True, sync_dist=True)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        y_hat = self.model(inputs)
        loss, cell_loss, vegf_loss, cell_constraint, vegf_constraint = self.criterion(y_hat, targets)
        self.log('val_loss', loss, on_step=False, on_epoch=True, sync_dist=True) #sync_dist=True is used for multi-GPU training
        ...
        self.log('val_cell_constraint', cell_constraint, on_epoch=True, sync_dist=True)
        self.log('val_vegf_constraint', vegf_constraint, on_epoch=True, sync_dist=True)
        ...
        self.log('cell_const/loss', cell_constraint/loss, on_step=False, on_epoch=True, sync_dist=True)
        self.log('vegf_const/loss', vegf_constraint/loss, on_step=False, on_epoch=True, sync_dist=True)
        
        # Only log for the first batch and on the main process in DDP
        if batch_idx == 0 and self.trainer.is_global_zero and self.current_epoch % 10 == 0:
...
def main():
    ...
    print('loss function weight three:', LOSS_WEIGHT_THREE)
    print('loss function weight four:', LOSS_WEIGHT_FOUR)
