...
train script for a recurrent, residual periodic u net with 2 loss terms in the loss function
Yielded a sub-optimal model. See Periodic_Unet_2_loss.py for the best model and the one described in the paper.
'''
from models.unet_pbc_prelu_split_channels import ResUNetPBCPrelu
...
class LitModelPeriodic(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Logging input-output pairs at defined epochs
        if batch_idx == 0 and self.trainer.is_global_zero and (self.current_epoch % 30 == 0 or self.current_epoch in [1, 5, 10, 99]):
            ...
            input_channel_0 = input_sample[0].unsqueeze(0)  # [1, H, W]
            input_channel_1 = input_sample[1].unsqueeze(0)  # [1, H, W]
            output_channel_0 = output_sample[0].unsqueeze(0)  # [1, H, W]
            output_channel_1 = output_sample[1].unsqueeze(0)  # [1, H, W]

            # Concatenate along the width (W) dimension
            input_concat = torch.cat([input_channel_0, input_channel_1], dim=-1)  # [1, H, 2W]
            output_concat = torch.cat([output_channel_0, output_channel_1], dim=-1)  # [1, H, 2W]

            # Concatenate along the height (H) dimension
            full_concat = torch.cat([input_concat, output_concat], dim=-2)  # [1, 2H, 2W]

            tag = f'Epoch_{self.current_epoch}_Input-Output'
            self.logger.experiment.add_image(tag, full_concat, self.current_epoch)
...
def main():
    ...
    #compiled_model = torch.compile(LitModelPeriodic(), mode="reduce-overhead")
    ...
    trainer = Trainer(
        ...
        detect_anomaly=False,  # Detect anomaly during training
