'''
train script for a periodic unet model with 3 loss terms in the loss function
...
from models.model import Dice_MSE_sseVEGF_loss, BCE_MSE_sseVEGF_loss
...
class LitModelPeriodic(pl.LightningModule):
    def __init__(self):
        ...
        # self.criterion = Dice_MSE_sseVEGF_loss(dice_weight=LOSS_WEIGHT_ONE, mse_weight=LOSS_WEIGHT_TWO, sseVEGF_weight=LOSS_WEIGHT_THREE)
        self.criterion = BCE_MSE_sseVEGF_loss(bce_weight=LOSS_WEIGHT_ONE, mse_weight=LOSS_WEIGHT_TWO, sseVEGF_weight=LOSS_WEIGHT_THREE)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss, cell_loss, vegf_loss, vegf_constraint = self.criterion(y_hat, targets)
